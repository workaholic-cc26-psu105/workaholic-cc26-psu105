  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-2 min-w-0 gap-2">
        {/* LEFT: tipe + judul + perusahaan */}
        <div className="min-w-0 flex-1">
          <span className="inline-block text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-[#FDF2F2] text-[#8B1A1A] mb-2 max-w-full truncate">
            {job.tipe}
          </span>

          <h4 className="font-extrabold text-gray-900 text-sm leading-tight break-words">
            {job.judul}
          </h4>
      ...
      <div className="flex items-center justify-between mt-3 gap-2 min-w-0">
        ...
        <span className="text-xs font-bold text-[#8B1A1A] whitespace-nowrap flex-shrink-0">
          {job.gaji}
        </span>
      </div>
    </div>
...
export default function CVReviewPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F0F0F0] font-sans antialiased overflow-x-hidden">
      <main className="max-w-5xl mx-auto w-full px-4 md:px-6 py-10 space-y-6 min-w-0">

        {/* HEADER */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 px-6 py-5 flex flex-col sm:flex-row sm:items-center gap-4 min-w-0 overflow-hidden">
        ...
        {/* KECOCOKAN */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 px-6 py-5 overflow-hidden">
          <div className="flex flex-col sm:flex-row gap-6">
            ...
            <div className="flex items-center gap-4 flex-1 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-base md:text-lg font-extrabold text-gray-900 leading-tight break-words">
            ...
            <div className="hidden sm:block w-px bg-gray-100 flex-shrink-0"></div>
        ...
        {/* GRID */}
        <div className="grid grid-cols-1 md:grid-cols-[1fr_1.8fr] gap-6 min-w-0">
              ...
              <div className="flex flex-wrap gap-2">
                {result.skills?.length > 0 ? (
                  result.skills.map((skill, index) => (
                    <div
                      key={`${skill}-${index}`}
                      className="px-3 py-2 rounded-xl bg-[#FDF2F2] text-[#8B1A1A] text-xs font-bold max-w-full break-words"
                    >
                      {skill}
                    </div>
                  ))
              ...
              <div className="flex flex-wrap gap-2">
                {result.kategori?.length > 0 ? (
                  result.kategori.map((kat, index) => (
                    <div
                      key={`${kat}-${index}`}
                      className="px-3 py-2 rounded-xl bg-blue-50 text-blue-700 text-xs font-bold max-w-full break-words"
                    >
                      {kat}
                    </div>
                  ))
              ...
              <div className="space-y-3">
                {result.saran?.length > 0 ? (
                  result.saran.map((item, index) => (
                    ...
                    >
                      <div className="mt-1.5 w-2 h-2 rounded-full bg-[#8B1A1A] flex-shrink-0"></div>
          ...
          {/* RIGHT */}
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 px-6 py-5 min-w-0 overflow-hidden">
            <div className="flex items-center justify-between mb-5">
              ...
              <button
                ...
                className="text-xs font-bold text-gray-400 hover:text-[#8B1A1A] flex-shrink-0"
              >
                Lihat semua
              </button>
            </div>
