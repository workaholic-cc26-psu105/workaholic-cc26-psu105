...
from tensorflow.keras.layers import Bidirectional as KerasBidirectional
...
for _cls in [KerasEmbedding, KerasDense, KerasDropout, KerasBidirectional]:
    _cls.from_config = _make_patch(_cls.from_config)
...
try:
    ...
    model       = tf.keras.models.load_model(str(MODELS_DIR / 'workaholic_bilstm.keras'), compile=False, safe_mode=False)
    embed_model = tf.keras.models.load_model(str(MODELS_DIR / 'workaholic_encoder.keras'), compile=False, safe_mode=False)
